...
import com.ytjojo.viewlib.supernestedlayout.SuperNestedLayout.LayoutParams;
...
public class LayoutManager {
    ...
    public LayoutManager(SuperNestedLayout parent) {
        this.mSuperNestedLayout = parent;
    }
    HashMap<String,Integer> mBehaviorViewValue = new HashMap<>();
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        prepareChildren();
        int childCount = mSuperNestedLayout.getChildCount();
        final WindowInsetsCompat lastInsets = mSuperNestedLayout.getLastInsets();
        final boolean applyInsets = lastInsets != null && ViewCompat.getFitsSystemWindows(mSuperNestedLayout);
        int horizInsets = 0;
        int vertInsets = 0;
        if (applyInsets) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (b != null && b.onMeasureChild(mSuperNestedLayout, child, widthMeasureSpec, 0,
                    heightMeasureSpec, 0)) {
                continue;
            }
    ...
    private void prepareChildren() {
        ...
        for (int i = 0, count = mSuperNestedLayout.getChildCount(); i < count; i++) {
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        mWidth = mSuperNestedLayout.getMeasuredWidth();
        mHeight = mSuperNestedLayout.getMeasuredHeight();
        mPaddingLeft = mSuperNestedLayout.getPaddingLeft();
        mPaddingTop = mSuperNestedLayout.getPaddingTop();
        mPaddingRight = mSuperNestedLayout.getPaddingRight();
        mPaddingBottom = mSuperNestedLayout.getPaddingBottom();
        final int layoutDirection = ViewCompat.getLayoutDirection(mSuperNestedLayout);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (behavior == null || !behavior.onLayoutChild(mSuperNestedLayout, child, layoutDirection)) {
...
//                mNestedScrollLayout.setMaxScrollY(Math.max(viewBehavior.getMaxScrollY(), mNestedScrollLayout.getMaxScrollY()));
    ...
    private int layoutChildVertical(View child, int childTop, int childWidthSpace) {
        ...
        if (childTop == mPaddingTop) {
            if (mSuperNestedLayout.getLastInsets() != null && ViewCompat.getFitsSystemWindows(mSuperNestedLayout)
                    && !ViewCompat.getFitsSystemWindows(child)) {
                childTop += mSuperNestedLayout.getLastInsets().getSystemWindowInsetTop();
            }
...
//                        height - mPaddingBottom - childHeight - lp.bottomMargin));
    ...
    void getDescendantRect(View descendant, Rect out, boolean onLayout) {
        ...
        if (onLayout) {
            mSuperNestedLayout.offsetDescendantRectToMyCoords(descendant, out);

        } else {
            offsetRectBetweenParentAndChild(mSuperNestedLayout, descendant, out);
    ...
    public void dispatchOnDependentViewChanged() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            for (int j = i + 1; j < childCount; j++) {
                ...
                if (b != null && b.layoutDependsOn(mSuperNestedLayout, checkChild, child)) {
                    final boolean handled = b.onDependentViewChanged(mSuperNestedLayout, checkChild, child);
    ...
    void offsetChildToAnchor(View child, int layoutDirection) {
        ...
        if (lp.mAnchorView != null) {
            ...
            if (dx != 0 || dy != 0) {
                ...
                final Behavior b = lp.getBehavior();
                if (b != null) {
                    b.onDependentViewChanged(mSuperNestedLayout, child, lp.mAnchorView);
                }
            }
        }
    }
     ...
     * {@link SuperNestedLayout} in the view hierarchy.
     *
