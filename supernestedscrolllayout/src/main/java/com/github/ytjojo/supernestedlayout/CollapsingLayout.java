...
public class CollapsingLayout extends FrameLayout {
    ...
    public CollapsingLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mCollapsingTextHelper.setTextSizeInterpolator(new DecelerateInterpolator());

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.SuperNested_CollapsingLayout, defStyleAttr,
                R.style.Widget_CollapsingLayout);

        mCollapsingTextHelper.setExpandedTextGravity(
                a.getInt(R.styleable.SuperNested_CollapsingLayout_expandedTitleLayoutGravity,
                        GravityCompat.START | Gravity.BOTTOM));
        mCollapsingTextHelper.setCollapsedTextGravity(
                a.getInt(R.styleable.SuperNested_CollapsingLayout_collapsedTitleLayoutGravity,
                        GravityCompat.START | Gravity.CENTER_VERTICAL));

        mExpandedMarginStart = mExpandedMarginTop = mExpandedMarginEnd = mExpandedMarginBottom =
                a.getDimensionPixelSize(R.styleable.SuperNested_CollapsingLayout_expandedTitleMargin, 0);

        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginStart)) {
            mExpandedMarginStart = a.getDimensionPixelSize(
                    R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginStart, 0);
        }
        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginEnd)) {
            mExpandedMarginEnd = a.getDimensionPixelSize(
                    R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginEnd, 0);
        }
        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginTop)) {
            mExpandedMarginTop = a.getDimensionPixelSize(
                    R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginTop, 0);
        }
        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginBottom)) {
            mExpandedMarginBottom = a.getDimensionPixelSize(
                    R.styleable.SuperNested_CollapsingLayout_expandedTitleMarginBottom, 0);
        }

        mCollapsingTitleEnabled = a.getBoolean(
                R.styleable.SuperNested_CollapsingLayout_titleEnabled, true);
        setTitle(a.getText(R.styleable.SuperNested_CollapsingLayout_title));

        // First load the default text appearances
        mCollapsingTextHelper.setExpandedTextAppearance(
                R.style.TextAppearance_Design_CollapsingLayout_Expanded);
        mCollapsingTextHelper.setCollapsedTextAppearance(
                R.style.TextAppearance_Design_CollapsingLayout_Collapsed);

        // Now overlay any custom text appearances
        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_expandedTitleTextAppearance)) {
            mCollapsingTextHelper.setExpandedTextAppearance(
                    a.getResourceId(
                            R.styleable.SuperNested_CollapsingLayout_expandedTitleTextAppearance, 0));
        }
        if (a.hasValue(R.styleable.SuperNested_CollapsingLayout_collapsedTitleTextAppearance)) {
            mCollapsingTextHelper.setCollapsedTextAppearance(
                    a.getResourceId(
                            R.styleable.SuperNested_CollapsingLayout_collapsedTitleTextAppearance, 0));
        }

        mScrimVisibleHeightTrigger = a.getDimensionPixelSize(
                R.styleable.SuperNested_CollapsingLayout_scrimVisibleHeightTrigger, -1);

        mScrimAnimationDuration = a.getInt(
                R.styleable.SuperNested_CollapsingLayout_scrimAnimationDuration,
                DEFAULT_SCRIM_ANIMATION_DURATION);

        setContentScrim(a.getDrawable(R.styleable.SuperNested_CollapsingLayout_contentScrim));
        setStatusBarScrim(a.getDrawable(R.styleable.SuperNested_CollapsingLayout_statusBarScrim));

        mToolbarId = a.getResourceId(R.styleable.SuperNested_CollapsingLayout_toolbarId, -1);
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this,
                new android.support.v4.view.OnApplyWindowInsetsListener() {
                    ...
                    public WindowInsetsCompat onApplyWindowInsets(View v,
                        ...
                        return setWindowInsets(insets,true);
                    }
                });
    }
    ...
    protected void onAttachedToWindow() {
        ...
        if (lp instanceof  SuperNestedLayout.LayoutParams) {
            ...
            SuperNestedLayout.LayoutParams layoutParams = (SuperNestedLayout.LayoutParams) lp;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mCollapsingTitleEnabled && mDummyView != null) {
            ...
            if (mDrawCollapsingTitle) {
                ...
                ViewGroupUtil.getDescendantRect(this, mToolbar, mTmpRect);
    ...
    public void setCollapsedTitleTextColor(@ColorInt int color) {
        mCollapsingTextHelper.setCollapsedTextColor(ColorStateList.valueOf(color));
    }
    /**
    ...
    public void setExpandedTitleColor(@ColorInt int color) {
        mCollapsingTextHelper.setExpandedTextColor(ColorStateList.valueOf(color));
    }
    /**
    ...
    public static class LayoutParams extends FrameLayout.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs,
                    R.styleable.SuperNested_CollapsingLayout_Layout);
            mCollapseMode = a.getInt(
                    R.styleable.SuperNested_CollapsingLayout_Layout_SuperNested_collapseMode,
                    COLLAPSE_MODE_OFF);
            setParallaxMultiplier(a.getFloat(
                    R.styleable.SuperNested_CollapsingLayout_Layout_layout_collapseParallaxMultiplier,
